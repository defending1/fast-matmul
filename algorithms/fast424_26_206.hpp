...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = -dataM1[i + static_cast<size_t>(j) * strideM1] + dataM2[i + static_cast<size_t>(j) * strideM2] -dataM3[i + static_cast<size_t>(j) * strideM3] + dataM4[i + static_cast<size_t>(j) * strideM4] + dataM5[i + static_cast<size_t>(j) * strideM5] + beta * dataC[i + static_cast<size_t>(j) * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = -dataM1[i + static_cast<size_t>(j) * strideM1] + dataM2[i + static_cast<size_t>(j) * strideM2] -dataM3[i + static_cast<size_t>(j) * strideM3] + dataM4[i + static_cast<size_t>(j) * strideM4] + dataM5[i + static_cast<size_t>(j) * strideM5];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = Scalar(-0.5) * dataM1[i + static_cast<size_t>(j) * strideM1] -dataM2[i + static_cast<size_t>(j) * strideM2] + dataM3[i + static_cast<size_t>(j) * strideM3] + beta * dataC[i + static_cast<size_t>(j) * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = Scalar(-0.5) * dataM1[i + static_cast<size_t>(j) * strideM1] -dataM2[i + static_cast<size_t>(j) * strideM2] + dataM3[i + static_cast<size_t>(j) * strideM3];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = Scalar(-0.5) * dataM1[i + static_cast<size_t>(j) * strideM1] + dataM2[i + static_cast<size_t>(j) * strideM2] + Scalar(0.5) * dataM3[i + static_cast<size_t>(j) * strideM3] + dataM4[i + static_cast<size_t>(j) * strideM4] + dataM5[i + static_cast<size_t>(j) * strideM5] + dataM6[i + static_cast<size_t>(j) * strideM6] + dataM7[i + static_cast<size_t>(j) * strideM7] + beta * dataC[i + static_cast<size_t>(j) * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = Scalar(-0.5) * dataM1[i + static_cast<size_t>(j) * strideM1] + dataM2[i + static_cast<size_t>(j) * strideM2] + Scalar(0.5) * dataM3[i + static_cast<size_t>(j) * strideM3] + dataM4[i + static_cast<size_t>(j) * strideM4] + dataM5[i + static_cast<size_t>(j) * strideM5] + dataM6[i + static_cast<size_t>(j) * strideM6] + dataM7[i + static_cast<size_t>(j) * strideM7];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = Scalar(0.5) * dataM1[i + static_cast<size_t>(j) * strideM1] + Scalar(0.5) * dataM2[i + static_cast<size_t>(j) * strideM2] + dataM3[i + static_cast<size_t>(j) * strideM3] + beta * dataC[i + static_cast<size_t>(j) * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = Scalar(0.5) * dataM1[i + static_cast<size_t>(j) * strideM1] + Scalar(0.5) * dataM2[i + static_cast<size_t>(j) * strideM2] + dataM3[i + static_cast<size_t>(j) * strideM3];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = Scalar(-0.5) * dataM1[i + static_cast<size_t>(j) * strideM1] + Scalar(-0.5) * dataM2[i + static_cast<size_t>(j) * strideM2] + dataM3[i + static_cast<size_t>(j) * strideM3] -dataM4[i + static_cast<size_t>(j) * strideM4] + beta * dataC[i + static_cast<size_t>(j) * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = Scalar(-0.5) * dataM1[i + static_cast<size_t>(j) * strideM1] + Scalar(-0.5) * dataM2[i + static_cast<size_t>(j) * strideM2] + dataM3[i + static_cast<size_t>(j) * strideM3] -dataM4[i + static_cast<size_t>(j) * strideM4];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = -dataM1[i + static_cast<size_t>(j) * strideM1] + dataM2[i + static_cast<size_t>(j) * strideM2] + Scalar(-0.5) * dataM3[i + static_cast<size_t>(j) * strideM3] -dataM4[i + static_cast<size_t>(j) * strideM4] -dataM5[i + static_cast<size_t>(j) * strideM5] + beta * dataC[i + static_cast<size_t>(j) * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = -dataM1[i + static_cast<size_t>(j) * strideM1] + dataM2[i + static_cast<size_t>(j) * strideM2] + Scalar(-0.5) * dataM3[i + static_cast<size_t>(j) * strideM3] -dataM4[i + static_cast<size_t>(j) * strideM4] -dataM5[i + static_cast<size_t>(j) * strideM5];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = dataM1[i + static_cast<size_t>(j) * strideM1] + Scalar(0.5) * dataM2[i + static_cast<size_t>(j) * strideM2] -dataM3[i + static_cast<size_t>(j) * strideM3] + beta * dataC[i + static_cast<size_t>(j) * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = dataM1[i + static_cast<size_t>(j) * strideM1] + Scalar(0.5) * dataM2[i + static_cast<size_t>(j) * strideM2] -dataM3[i + static_cast<size_t>(j) * strideM3];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = dataSX1[i + static_cast<size_t>(j) * strideSX1] + dataSX2[i + static_cast<size_t>(j) * strideSX2];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = dataTX1[i + static_cast<size_t>(j) * strideTX1] + dataTX2[i + static_cast<size_t>(j) * strideTX2];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = -dataMX1[i + static_cast<size_t>(j) * strideMX1] + dataMX2[i + static_cast<size_t>(j) * strideMX2];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = dataMX1[i + static_cast<size_t>(j) * strideMX1] + Scalar(0.5) * dataMX2[i + static_cast<size_t>(j) * strideMX2];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = Scalar(-0.5) * dataMX1[i + static_cast<size_t>(j) * strideMX1] + dataMX2[i + static_cast<size_t>(j) * strideMX2];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = Scalar(0.5) * dataMX1[i + static_cast<size_t>(j) * strideMX1] + Scalar(-0.5) * dataMX2[i + static_cast<size_t>(j) * strideMX2];
        }
    }
}
...
# pragma omp task if(sequential26) shared(mem_mngr, locker) untied default(shared)
    ...
    M26.UpdateMultiplier(Scalar(-1.0));
