...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = dataS1[i + static_cast<size_t>(j) * strideS1] -dataS2[i + static_cast<size_t>(j) * strideS2] + dataS3[i + static_cast<size_t>(j) * strideS3] -dataS4[i + static_cast<size_t>(j) * strideS4] + dataS5[i + static_cast<size_t>(j) * strideS5] -dataS6[i + static_cast<size_t>(j) * strideS6] + dataS7[i + static_cast<size_t>(j) * strideS7] -dataS8[i + static_cast<size_t>(j) * strideS8];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = Scalar(0.5) * dataS1[i + static_cast<size_t>(j) * strideS1] -dataS2[i + static_cast<size_t>(j) * strideS2] + Scalar(0.5) * dataS3[i + static_cast<size_t>(j) * strideS3] + Scalar(0.5) * dataS4[i + static_cast<size_t>(j) * strideS4];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = -dataS1[i + static_cast<size_t>(j) * strideS1] + dataS2[i + static_cast<size_t>(j) * strideS2] -dataS3[i + static_cast<size_t>(j) * strideS3] + dataS4[i + static_cast<size_t>(j) * strideS4] -dataS5[i + static_cast<size_t>(j) * strideS5] + dataS6[i + static_cast<size_t>(j) * strideS6] -dataS7[i + static_cast<size_t>(j) * strideS7];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = -dataS1[i + static_cast<size_t>(j) * strideS1];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = -dataS1[i + static_cast<size_t>(j) * strideS1] -dataS2[i + static_cast<size_t>(j) * strideS2] + dataS3[i + static_cast<size_t>(j) * strideS3] + dataS4[i + static_cast<size_t>(j) * strideS4];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = dataS1[i + static_cast<size_t>(j) * strideS1] + dataS2[i + static_cast<size_t>(j) * strideS2] + dataS3[i + static_cast<size_t>(j) * strideS3];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = dataS1[i + static_cast<size_t>(j) * strideS1] -dataS2[i + static_cast<size_t>(j) * strideS2] + dataS3[i + static_cast<size_t>(j) * strideS3] -dataS4[i + static_cast<size_t>(j) * strideS4] + dataS5[i + static_cast<size_t>(j) * strideS5] -dataS6[i + static_cast<size_t>(j) * strideS6];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = -dataS1[i + static_cast<size_t>(j) * strideS1] -dataS2[i + static_cast<size_t>(j) * strideS2];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = Scalar(-0.5) * dataS1[i + static_cast<size_t>(j) * strideS1] + Scalar(0.5) * dataS2[i + static_cast<size_t>(j) * strideS2] + Scalar(-0.5) * dataS3[i + static_cast<size_t>(j) * strideS3] -dataS4[i + static_cast<size_t>(j) * strideS4];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = dataT1[i + static_cast<size_t>(j) * strideT1] + dataT2[i + static_cast<size_t>(j) * strideT2];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = -dataT1[i + static_cast<size_t>(j) * strideT1] + dataT2[i + static_cast<size_t>(j) * strideT2] -dataT3[i + static_cast<size_t>(j) * strideT3];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = dataT1[i + static_cast<size_t>(j) * strideT1] + dataT2[i + static_cast<size_t>(j) * strideT2] + dataT3[i + static_cast<size_t>(j) * strideT3] + dataT4[i + static_cast<size_t>(j) * strideT4];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = -dataT1[i + static_cast<size_t>(j) * strideT1];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = dataT1[i + static_cast<size_t>(j) * strideT1] -dataT2[i + static_cast<size_t>(j) * strideT2] -dataT3[i + static_cast<size_t>(j) * strideT3] -dataT4[i + static_cast<size_t>(j) * strideT4] + dataT5[i + static_cast<size_t>(j) * strideT5] + dataT6[i + static_cast<size_t>(j) * strideT6];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + static_cast<size_t>(j) * strideC] = dataT1[i + static_cast<size_t>(j) * strideT1] -dataT2[i + static_cast<size_t>(j) * strideT2] -dataT3[i + static_cast<size_t>(j) * strideT3] -dataT4[i + static_cast<size_t>(j) * strideT4] + dataT5[i + static_cast<size_t>(j) * strideT5];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = dataM1[i + static_cast<size_t>(j) * strideM1] -dataM2[i + static_cast<size_t>(j) * strideM2] -dataM3[i + static_cast<size_t>(j) * strideM3] -dataM4[i + static_cast<size_t>(j) * strideM4] + beta * dataC[i + static_cast<size_t>(j) * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = dataM1[i + static_cast<size_t>(j) * strideM1] -dataM2[i + static_cast<size_t>(j) * strideM2] -dataM3[i + static_cast<size_t>(j) * strideM3] -dataM4[i + static_cast<size_t>(j) * strideM4];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = -dataM1[i + static_cast<size_t>(j) * strideM1] + dataM2[i + static_cast<size_t>(j) * strideM2] + beta * dataC[i + static_cast<size_t>(j) * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = -dataM1[i + static_cast<size_t>(j) * strideM1] + dataM2[i + static_cast<size_t>(j) * strideM2];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = dataM1[i + static_cast<size_t>(j) * strideM1] + Scalar(0.5) * dataM2[i + static_cast<size_t>(j) * strideM2] + Scalar(-0.5) * dataM3[i + static_cast<size_t>(j) * strideM3] + dataM4[i + static_cast<size_t>(j) * strideM4] + beta * dataC[i + static_cast<size_t>(j) * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = dataM1[i + static_cast<size_t>(j) * strideM1] + Scalar(0.5) * dataM2[i + static_cast<size_t>(j) * strideM2] + Scalar(-0.5) * dataM3[i + static_cast<size_t>(j) * strideM3] + dataM4[i + static_cast<size_t>(j) * strideM4];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = dataM1[i + static_cast<size_t>(j) * strideM1] -dataM2[i + static_cast<size_t>(j) * strideM2] + Scalar(0.5) * dataM3[i + static_cast<size_t>(j) * strideM3] + Scalar(0.5) * dataM4[i + static_cast<size_t>(j) * strideM4] -dataM5[i + static_cast<size_t>(j) * strideM5] + dataM6[i + static_cast<size_t>(j) * strideM6] -dataM7[i + static_cast<size_t>(j) * strideM7] + beta * dataC[i + static_cast<size_t>(j) * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = dataM1[i + static_cast<size_t>(j) * strideM1] -dataM2[i + static_cast<size_t>(j) * strideM2] + Scalar(0.5) * dataM3[i + static_cast<size_t>(j) * strideM3] + Scalar(0.5) * dataM4[i + static_cast<size_t>(j) * strideM4] -dataM5[i + static_cast<size_t>(j) * strideM5] + dataM6[i + static_cast<size_t>(j) * strideM6] -dataM7[i + static_cast<size_t>(j) * strideM7];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = Scalar(0.5) * dataM1[i + static_cast<size_t>(j) * strideM1] + dataM2[i + static_cast<size_t>(j) * strideM2] + Scalar(-0.5) * dataM3[i + static_cast<size_t>(j) * strideM3] + Scalar(-0.5) * dataM4[i + static_cast<size_t>(j) * strideM4] + Scalar(-0.5) * dataM5[i + static_cast<size_t>(j) * strideM5] -dataM6[i + static_cast<size_t>(j) * strideM6] + dataM7[i + static_cast<size_t>(j) * strideM7] -dataM8[i + static_cast<size_t>(j) * strideM8] -dataM9[i + static_cast<size_t>(j) * strideM9] + Scalar(0.5) * dataM10[i + static_cast<size_t>(j) * strideM10] + Scalar(0.5) * dataM11[i + static_cast<size_t>(j) * strideM11] + beta * dataC[i + static_cast<size_t>(j) * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = Scalar(0.5) * dataM1[i + static_cast<size_t>(j) * strideM1] + dataM2[i + static_cast<size_t>(j) * strideM2] + Scalar(-0.5) * dataM3[i + static_cast<size_t>(j) * strideM3] + Scalar(-0.5) * dataM4[i + static_cast<size_t>(j) * strideM4] + Scalar(-0.5) * dataM5[i + static_cast<size_t>(j) * strideM5] -dataM6[i + static_cast<size_t>(j) * strideM6] + dataM7[i + static_cast<size_t>(j) * strideM7] -dataM8[i + static_cast<size_t>(j) * strideM8] -dataM9[i + static_cast<size_t>(j) * strideM9] + Scalar(0.5) * dataM10[i + static_cast<size_t>(j) * strideM10] + Scalar(0.5) * dataM11[i + static_cast<size_t>(j) * strideM11];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = Scalar(0.5) * dataM1[i + static_cast<size_t>(j) * strideM1] + Scalar(-0.5) * dataM2[i + static_cast<size_t>(j) * strideM2] + Scalar(0.5) * dataM3[i + static_cast<size_t>(j) * strideM3] + Scalar(0.5) * dataM4[i + static_cast<size_t>(j) * strideM4] + beta * dataC[i + static_cast<size_t>(j) * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = Scalar(0.5) * dataM1[i + static_cast<size_t>(j) * strideM1] + Scalar(-0.5) * dataM2[i + static_cast<size_t>(j) * strideM2] + Scalar(0.5) * dataM3[i + static_cast<size_t>(j) * strideM3] + Scalar(0.5) * dataM4[i + static_cast<size_t>(j) * strideM4];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = Scalar(0.5) * dataM1[i + static_cast<size_t>(j) * strideM1] + Scalar(-0.5) * dataM2[i + static_cast<size_t>(j) * strideM2] + Scalar(-0.5) * dataM3[i + static_cast<size_t>(j) * strideM3] + Scalar(0.5) * dataM4[i + static_cast<size_t>(j) * strideM4] + Scalar(-0.5) * dataM5[i + static_cast<size_t>(j) * strideM5] + Scalar(-0.5) * dataM6[i + static_cast<size_t>(j) * strideM6];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = -dataM1[i + static_cast<size_t>(j) * strideM1] + dataM2[i + static_cast<size_t>(j) * strideM2] + Scalar(0.5) * dataM3[i + static_cast<size_t>(j) * strideM3] + Scalar(-0.5) * dataM4[i + static_cast<size_t>(j) * strideM4] + Scalar(-0.5) * dataM5[i + static_cast<size_t>(j) * strideM5] + Scalar(0.5) * dataM6[i + static_cast<size_t>(j) * strideM6] -dataM7[i + static_cast<size_t>(j) * strideM7] + beta * dataC[i + static_cast<size_t>(j) * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + static_cast<size_t>(j) * strideC] = -dataM1[i + static_cast<size_t>(j) * strideM1] + dataM2[i + static_cast<size_t>(j) * strideM2] + Scalar(0.5) * dataM3[i + static_cast<size_t>(j) * strideM3] + Scalar(-0.5) * dataM4[i + static_cast<size_t>(j) * strideM4] + Scalar(-0.5) * dataM5[i + static_cast<size_t>(j) * strideM5] + Scalar(0.5) * dataM6[i + static_cast<size_t>(j) * strideM6] -dataM7[i + static_cast<size_t>(j) * strideM7];
            }
        }
    }
}
...
# pragma omp task if(sequential5) shared(mem_mngr, locker) untied default(shared)
    ...
    M5.UpdateMultiplier(Scalar(-1.0));
...
# pragma omp task if(sequential12) shared(mem_mngr, locker) untied default(shared)
    {
#endif
    M12.UpdateMultiplier(Scalar(-1.0));
    M12.UpdateMultiplier(Scalar(1.0));
...
# pragma omp task if(sequential13) shared(mem_mngr, locker) untied default(shared)
    ...
    M13.UpdateMultiplier(Scalar(-1.0));
...
# pragma omp task if(sequential19) shared(mem_mngr, locker) untied default(shared)
    ...
    M19.UpdateMultiplier(Scalar(-1.0));
...
# pragma omp task if(sequential23) shared(mem_mngr, locker) untied default(shared)
    ...
    M23.UpdateMultiplier(Scalar(-1.0));
