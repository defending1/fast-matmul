...
# Set style
latex_installed = shutil.which("latex") is not None or shutil.which("pdflatex") is not None
if latex_installed:
...
def main():
    ...
    csv_path = os.path.join(project_root, "generated", "csv", "run1", "benchmark_results.csv")
    base_csv_path = os.path.join(project_root, "generated", "csv", "run1", "benchmark_results_base.csv")
    ballard_path = os.path.join(project_root, "benchmarks", "generated", "benchmarks_seq.txt")
    ...
    ballard_strassen_levels = [ballard_strassen_1, ballard_strassen_2, ballard_strassen_3]
    ...
    for row, col, title, is_cutoff, value in configs:
        ...
        l_mkl, = ax.plot(n_base, gflops_mkl, label="MKL (Sequential)", color="#9467bd", marker="p", linestyle="--", linewidth=1.2, markersize=4.5)
        legend_handles["MKL (Sequential)"] = l_mkl
        
        # 2. Plot faer Base (Sequential)
        gflops_faer = mkl_flops / (df_base["faer_seq"] * 1e9)
        l_faer, = ax.plot(n_base, gflops_faer, label="faer (Sequential)", color="#17becf", marker="d", linestyle="-", linewidth=1.2, markersize=4.5)
        ...
        if not df_dgemm.empty:
            ...
            l_strassen_dg, = ax.plot(df_dgemm["size"], gflops_dg, label="Strassen (dgemm base)", color="#ff7f0e", marker="s", linestyle="--", linewidth=1.2, markersize=4.5)
        ...
        if not df_faer_strassen.empty:
            ...
            l_strassen_fs, = ax.plot(df_faer_strassen["size"], gflops_fs, label="Strassen (faer base)", color="#e34a33", marker="o", linestyle="-", linewidth=1.2, markersize=4.5)
        ...
        if ballard_mkl is not None:
            ...
            l_b_mkl, = ax.plot(n_b, gflops_b, label="MKL (Ballard)", color="#e41a1c", marker="X", linestyle="--", linewidth=1.5, markersize=5.0)
    ...
    sorted_labels = [
        "MKL (Sequential)", "faer (Sequential)", 
        "Strassen (dgemm base)", "Strassen (faer base)", 
        "MKL (Ballard)", "Strassen L1 (Ballard)", 
        "Strassen L2 (Ballard)", "Strassen L3 (Ballard)"
    ]
    ...
    plt.suptitle("Sequential Matrix Multiplication Performance Comparison\nRow 1: Cutoffs | Row 2: Recursion Levels", fontsize=15, y=0.98)
    
    plt.tight_layout()
    fig.subplots_adjust(hspace=0.25, wspace=0.15, top=0.90, bottom=0.08, left=0.06, right=0.96)
